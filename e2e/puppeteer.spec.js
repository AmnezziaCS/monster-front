...
async function run() {
  ...
  try {
    ...
    // 3) Cliquer sur quelques catégories puis revenir
    for (let i = 0; i < 3; i++) {
      const typeHandles = await page.$$("main ul li a, main .container ul li a, ul li a");
    ...
    // 4) Retour accueil via le logo
    await clickByText(page, "a", "Monster Front");
    await page.waitForFunction(() =>
      document.body.textContent.includes("Catalogue Monster")
    );

    // 4.1) Cliquer sur un ou deux monsters aléatoires
    ...
    for (const link of footerLinks) {
      ...
      try {
        ...
        if (isExternal) {
          ...
          // Vérifie qu’on est toujours sur le site
